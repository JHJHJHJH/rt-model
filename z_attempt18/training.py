...
features = ['hour_of_day', 'is_weekend', 'is_business_hour','is_holiday','temperature','humidity', 'rain', 'wind']
...
params = {
    ...
    'n_estimators': 10000      # Let early stopping find the best number
}
...
xgb.plot_importance(model_a, importance_type='cover', ax=ax) # 'weight' is the number of times a feature is used
plt.show()
...
booster_building_a = model_a # If it's an XGBRegressor
# 2. Prepare DMatrix for Building B's TRAINING data
# dtrain_b = xgb.DMatrix(X_train, label=y_train, feature_names=X_train.columns.tolist())
