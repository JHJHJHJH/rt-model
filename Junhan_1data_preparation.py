...
def calculate_cooling_load(df):
    ...
    if 'CHR-01-KW' in df.columns and 'CHR-02-KW' in df.columns and 'CHR-03-KW' in df.columns and 'CHR-04-KW' in df.columns:
...
def create_weather_features(df):
    ...
    df = utils.generate_weather_feature_interpolated(df)
...
def create_rain_wind_features(df):
    df = df.copy()
...
print("Starting Building A data preparation..")
...
df = create_rain_wind_features(df)
df = utils.generate_weather_lag_features(df)
print( "Final Building A shape : " + str(df.shape) )

parent = os.path.dirname(__file__)
df.to_csv(os.path.join(parent, 'Building_A.csv'), index=False)

print("Starting Building B data preparation..")
...
df_b = utils.generate_workday_feature(df_b)
df_b = utils.generate_weather_lag_features(df_b)

print( "Final Building B shape : " + str(df_b.shape) )
parent = os.path.dirname(__file__)
